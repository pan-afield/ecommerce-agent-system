...
import { getSessionAgentCoreAuthorization, refreshSession } from "@/lib/server-auth";
...
export async function GET() {
  ...
  if (response.status === 401) {
    ...
    if (!refreshed) return NextResponse.json({ error: { code: "auth_refresh_invalid", message: "登录状态已失效，请重新登录。" } }, { status: 401 });
