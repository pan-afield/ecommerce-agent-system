...
- After the user finishes a step, read the actual change and continue to the next teaching step without automatically running the full checks. Run Pytest, Ruff, and changed-file Mypy together after a bounded, observable feature slice is complete. If a step changes a security, concurrency, transaction, idempotency, migration, or data-integrity boundary, add and run the focused risk test as soon as that boundary is complete. Explain errors from evidence and let the user correct backend business code.
- Keep the teaching interaction incremental even when verification is deferred: give exactly one small coding step at a time, wait for the user's completion report, and do not ask the user to implement an entire feature slice in one response.
